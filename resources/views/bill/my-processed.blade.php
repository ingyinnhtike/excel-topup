...
@section('title','bill-processed')
@section('content')

<section class="section">
    <div class="card">
        <div class="card-header">
            <h4 class="card-title">My Processed Bill</h4>
        </div>
        <div class="card-body">
            <form role="form" action="{{ url('bill-filter') }}" method="post">
                @csrf
                <div class="row">
                    <div class="col-sm-4">
                        <h6>Start Date</h6>
                    ...
                    <div class="col-sm-4 mt-4">
                        ...
                        <a type="submit" class="btn btn-success" href="{{ route('my_processed_bill.export') }}" style="font-weight: bold;margin-left: 10px;">Export</a>
                    </div>
                </div>
            </form>
            <div class="table-responsive">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        @elseif($data->status == 'error')
                        <td><span class="label label-danger">{{ $data->status }}</span></td>
                        <td>internal server error because of internet connection</td>
                        @else
                        <td><span class="label label-info">{{ $data->status }}</span></td>
                        <td>other</td>
                        @endif
